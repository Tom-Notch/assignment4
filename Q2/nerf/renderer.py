...
from meshutils import decimate_mesh
from meshutils import poisson_mesh_reconstruction
...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        aabb_train = torch.FloatTensor(
            [-self.bound, -self.bound, -self.bound, self.bound, self.bound, self.bound]
        )
    ...
    def run(
        ...
        **kwargs,
        ...
        if light_d is None:
            ...
            light_d = safe_normalize(
                rays_o + torch.randn(3, device=rays_o.device)
            )  # [N, 3]

        # print(f'nears = {nears.min().item()} ~ {nears.max().item()}, fars = {fars.min().item()} ~ {fars.max().item()}')

        z_vals = torch.linspace(0.0, 1.0, self.opt.num_steps, device=device).unsqueeze(
            0
        )  # [1, T]
        ...
        if self.opt.upsample_steps > 0:
            with torch.no_grad():
                ...
                alphas = 1 - torch.exp(
                    -deltas * density_outputs["sigma"].squeeze(-1)
                )  # [N, T]
        ...
        if self.training:
            if self.opt.lambda_orient > 0 and normals is not None:
                # orientation loss
                loss_orient = (
                    weights.detach() * (normals * dirs).sum(-1).clamp(min=0) ** 2
                )
    ...
    def render(
        self, rays_o, rays_d, mvp, h, w, staged=False, max_ray_batch=4096, **kwargs
    ):
        # rays_o, rays_d: [B, N, 3]
