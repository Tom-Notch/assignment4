...
class _grid_encode(Function):
    ...
    def forward(
        ...
        max_level = (
            L if max_level is None else max(min(int(math.ceil(max_level * L)), L), 1)
        )
...
class GridEncoder(nn.Module):
    ...
    def grad_weight_decay(self, weight=0.1):
        ...
        _backend.grad_weight_decay(
            self.embeddings, self.embeddings.grad, self.offsets, weight, B, C, L
        )
