...
def run_test_1():
    ...
    """
    gaussians = Gaussians(
        num_points=10000, init_type="random", device="cpu", isotropic=True
    )

    scales = torch.tensor([[1.0057], [0.5317], [1.2069], [2.0091], [-1.0004]]).to(
        torch.float32
    )
...
def run_test_2():
    ...
    """
    gaussians = Gaussians(
        num_points=10000, init_type="random", device="cpu", isotropic=False
    )
    ...
    gt_answer = torch.tensor(
        [
            [[2618.8611, 5941.3306], [5941.3301, 14066.5830]],
            [[1762.7010, 1096.4006], [1096.4005, 1337.9269]],
            [[2478.8181, 339.9595], [339.9595, 992.3395]],
            [[1645.8475, -170.6580], [-170.6579, 233.4286]],
            [[623.9609, -285.9298], [-285.9297, 4030.9944]],
        ]
    ).to(torch.float32)
