...
from data_utils_harder_scene import trivial_collate
...
def ndc_to_screen_camera(camera, img_size=(128, 128)):
    ...
    screen_principal = torch.tensor([[img_size[0] / 2, img_size[1] / 2]]).to(
        torch.float32
    )
    ...
    return PerspectiveCameras(
        ...
        T=camera.T,
        in_ndc=False,
        focal_length=screen_focal,
        principal_point=screen_principal,
        image_size=(img_size,),
    )
...
def run_training(args):
    ...
    gt_viz_imgs = [
        (train_dataset[i]["image"] * 255.0).numpy().astype(np.uint8) for i in viz_idxs
    ]
    gt_viz_imgs = [np.array(Image.fromarray(x).resize((256, 256))) for x in gt_viz_imgs]
    gt_viz_img = np.concatenate(gt_viz_imgs, axis=1)

    viz_cameras = [
        ndc_to_screen_camera(train_dataset[i]["camera"]).cuda() for i in viz_idxs
    ]

    # Init gaussians and scene
    gaussians = Gaussians(
        num_points=10000, init_type="random", device=args.device, isotropic=True
    )
    ...
    for itr in range(args.num_itrs):
        ...
        if itr % args.viz_freq == 0:
            viz_frame = visualize_renders(scene, gt_viz_img, viz_cameras, (128, 128))
...
def get_args():
    ...
    parser.add_argument(
        "--data_path", default="./data/materials", type=str, help="Path to the dataset."
    )
    parser.add_argument(
        "--gaussians_per_splat",
        default=-1,
